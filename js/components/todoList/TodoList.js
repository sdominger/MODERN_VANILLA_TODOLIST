...
export default class {
    ...
    add () {
        ...
        let html = document.querySelector(`li[data-id="${x}"]`);
        console.log('li:'+li);
        ...
        // this.elt.querySelector('.todo-list').innerHTML = newTodo.render() + this.elt.querySelector('.todo-list').innerHTML;
    ...
    /*  let li = document.querySelector(`li[data-id="${id}"]`);
        ...
        console.error(label);
        console.error(JSON.stringify(todo));
    */
        let todo = this.getTodo(id);
        todo.check();
    }
